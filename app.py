...
from src.config import CLIENT_SECRETS_FILE, FLASK_DEBUG, settings
...
if __name__ == '__main__':
    ...
    if is_development:
        ...
        if not pathlib.Path(CLIENT_SECRETS_FILE).exists():
            ...
            print(
                "You'll need to create a project in Google Cloud Console and download the client secret."
            )
    ...
    has_client_secrets = (
        settings.get('CLIENT_SECRETS_JSON') is not None
        or pathlib.Path(CLIENT_SECRETS_FILE).exists()
    )
